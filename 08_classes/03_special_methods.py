class Employee:
  def __init__(self, name, employee_id, department, salary, start_date):
    self.name = name
    self.employee_id = employee_id
    self.department = department
    self.salary = salary
    self.start_date = start_date
    self.skills = []
  
  def __str__(self):
    return f"{self.name} ({self.employee_id}) - {self.department}"

  def give_raise(self, percent):
    """Give employee raise by given percent (10 rather than 0.1)"""
    self.salary = self.salary * (1 + percent / 100)
  
  def add_skill(self, skill):
    self.skills.append(skill)

  def __len__(self):
    return len(self.name)

  def has_skill(self, skill):
    return skill in self.skills

  def __contains__(self, skill):
    return skill in self.skills

  def __iter__(self):
    return iter(self.skills)

print(len("Kevin"))
print(len([1,2,3])) 

employee = Employee("Alice", "EMP001", "Engineering", 75000, "2024-01-01")
print(len(employee))  # protocol
employee.add_skill("Python")
employee.add_skill("SQL")
employee.add_skill("Data")

if employee.has_skill("Python"):
  print("1. This employee has Python skills") 

if "Python" in employee:
  print("2. This employee has Python skills")

for item in employee.skills:
  print(item)

for item in employee:
  print(item)